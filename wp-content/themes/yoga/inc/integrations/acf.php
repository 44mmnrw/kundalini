...
if (!function_exists('yoga_register_practice_card_fields')) {
	/**
	 * Fields used by practice cards in the library and search results.
	 */
	function yoga_register_practice_card_fields(): void {
		if (!function_exists('acf_add_local_field_group')) {
			return;
		}
		...
		acf_add_local_field_group(
			array(
				'key'        => 'group_practice_card_fields',
				'title'      => 'Карточка практики',
				'fields'     => array(
					array(
						'key'          => 'field_practice_short_description',
						'label'        => 'Краткое описание',
						'name'         => 'short_description',
						'type'         => 'textarea',
						'instructions' => 'Текст под названием практики в списке «Крийи» и в результатах поиска.',
						'rows'         => 3,
						'new_lines'    => '',
					),
				),
...
add_action('acf/init', 'yoga_register_practice_card_fields', 15);

if (!function_exists('yoga_load_legacy_practice_short_description')) {
	/**
	 * Show the description currently generated by WordPress for legacy
	 * practices until the new field is saved for the first time.
	 *
	 * @param mixed $value   Current ACF value.
	 * @param mixed $post_id ACF post identifier.
	 * @return mixed
	 */
	function yoga_load_legacy_practice_short_description($value, $post_id) {
		if (!is_admin() || !is_numeric($post_id)) {
			return $value;
		}

		$post_id = (int) $post_id;
		if ($post_id <= 0 || get_post_type($post_id) !== 'practice' || metadata_exists('post', $post_id, 'short_description')) {
			return $value;
		}
...
add_filter(
	'acf/load_value/key=field_practice_short_description',
	'yoga_load_legacy_practice_short_description',
	10,
	2
);
