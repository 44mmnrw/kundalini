...
function yoga_sadhana_get_latest_completed(int $user_id, int $practice_id): ?array {
	...
	$id = (int) $wpdb->get_var($wpdb->prepare(
		"SELECT id FROM {$table} WHERE user_id = %d AND practice_id = %d ORDER BY id DESC LIMIT 1",
		$user_id,
		$practice_id
	));
	$row = $id > 0 ? yoga_sadhana_get($id, $user_id) : null;
...
function yoga_sadhana_mark_day(int $user_id, int $sadhana_id): array|WP_Error {
	...
	} elseif (in_array($result['completed_days'], kundalini_sadhanas_progress_milestones($result['target_days']), true)) {
